...
describe("normalizeEndpoint", () => {
  ...
  test("strips trailing slashes", () => {
    expect(normalizeEndpoint("http://localhost:20128///")).toBe(
      "http://localhost:20128",
    );
  });
  test("assumes http for scheme-less input", () => {
    expect(normalizeEndpoint("localhost:20128")).toBe("http://localhost:20128");
    expect(normalizeEndpoint("192.168.1.5:20128/")).toBe(
      "http://192.168.1.5:20128",
    );
  });
  test("keeps explicit scheme", () => {
    expect(normalizeEndpoint("https://router.example.com")).toBe(
...
describe("isSyncStale", () => {
  ...
  test("fresh is not stale, old is stale", () => {
    ...
    expect(
      isSyncStale(new Date(Date.now() - 25 * 3600_000).toISOString()),
    ).toBe(true);
  });
});
...
describe("saveJsonMerge / loadJsonFile", () => {
  ...
  test("merges capabilities one level deep", () => {
    ...
    writeFileSync(
      path,
      JSON.stringify({
        capabilities: { image: { enabled: true }, tts: { enabled: false } },
      }),
    );

    const out = saveJsonMerge(
      { capabilities: { image: { enabled: false, model: "m" } } },
      path,
    );
  ...
  test("union-merges infoMissing key-wise (negative cache survives concurrent writes)", () => {
    ...
    writeFileSync(
      path,
      JSON.stringify({ infoMissing: { "a/kept": now, "a/old": now - 1000 } }),
    );

    const out = saveJsonMerge(
      { infoMissing: { "a/old": now, "b/fresh": now } },
      path,
    );
  ...
  test("prunes expired infoMissing entries (24 h TTL) on write, keeps fresh", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        infoMissing: {
          "a/fresh": now,
          "a/expired": now - INFO_MISSING_TTL - 1,
        },
      }),
    );
  ...
  test("strips voice/ffmpegPath but preserves stt capability state", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        voice: "x",
        ffmpegPath: "y",
        capabilities: { stt: { enabled: true }, image: { enabled: true } },
      }),
    );
...
describe("listRowIsRich", () => {
  ...
  test("thin rows are not rich", () => {
    expect(listRowIsRich({ id: "m", owned_by: "x" } as RemoteModel)).toBe(
      false,
    );
  });
  test("non-empty string array is rich", () => {
    expect(
      listRowIsRich({ id: "m", capabilities: ["vision"] } as RemoteModel),
    ).toBe(true);
  });
...
describe("capsClassOf / capsBadgeOf", () => {
  test("live caps → rich", () => {
    expect(
      capsClassOf({ id: "m", name: "m", kind: "chat", contextWindow: 1000 }),
    ).toBe("rich");
    expect(
      capsBadgeOf({ id: "m", name: "m", kind: "chat", contextWindow: 1000 }),
    ).toContain("live server info");
  });
  ...
  test("no caps → missing", () => {
    ...
    expect(capsBadgeOf({ id: "m", name: "m", kind: "image" })).toContain(
      "missing",
    );
  });
});
...
describe("isInfoCacheFresh", () => {
  test("fresh within 7 d, expired after", () => {
    const now = Date.now();
    expect(isInfoCacheFresh({ fetchedAt: now, info: { id: "a/m" } }, now)).toBe(
      true,
    );
    expect(
      isInfoCacheFresh(
        { fetchedAt: now - INFO_CACHE_TTL - 1, info: { id: "a/m" } },
        now,
      ),
    ).toBe(false);
...
describe("usage log (append-only jsonl)", () => {
  test("round-trips records, skips corrupt lines, summarizes", () => {
    ...
    logUsage(
      { tool: "nr_image_generate", model: "m/a", ms: 1000, ok: true, count: 2 },
      path,
    );
  ...
  test("never throws on bad paths", () => {
    expect(() =>
      logUsage(
        { tool: "t", ok: true },
        join(tmpdir(), "nr-test-\0bad", "u.jsonl"),
      ),
    ).not.toThrow();
  ...
  test("usage file stays bounded: an append past the size cap prunes to the tail", () => {
    ...
    logUsage(
      { tool: "nr_image_generate", model: "m/new", ms: 5, ok: true },
      path,
    );
...
describe("saveJsonMerge modelNames union-merge", () => {
  ...
  test("union-merges infoCache key-wise (merge — fresh on-disk records survive)", () => {
    ...
    const out = saveJsonMerge(
      { infoCache: { "chat\0a/new": mkRec("New") } },
      path,
    );
  ...
  test("patch wins per key for infoCache", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        infoCache: {
          "chat\0a/m": { fetchedAt: now, info: { id: "a/m", name: "Old" } },
        },
      }),
    );
    const out = saveJsonMerge(
      {
        infoCache: {
          "chat\0a/m": { fetchedAt: now + 1, info: { id: "a/m", name: "New" } },
        },
      },
      path,
    );
    const cache = out.infoCache as Record<
      string,
      { fetchedAt: number; info: { name: string } }
    >;
...
describe("saveJsonMerge prunes non-numeric timestamps", () => {
  ...
  test("malformed blobs do not resurrect through merge -> reload", () => {
    ...
    expect("b/nullinfo" in (out.infoCache as Record<string, unknown>)).toBe(
      false,
    );
      ...
      sanitizeInfoCache(
        {
          "chat\0a/good": {
            fetchedAt: now,
            info: { id: "a/good", name: "Good" },
          },
          "chat\0a/old": {
            fetchedAt: now - INFO_CACHE_TTL - 1,
            info: { id: "a/old" },
          },
          ...
          string,
          { fetchedAt: number; info: RemoteModel }
        >,
        now,
      ) ?? {};
...
describe("formatUsageSummary edges", () => {
  test("single call uses the singular, sub-second stays in ms", () => {
    ...
    expect(formatUsageSummary(path)).toBe(
      "Usage: 1 call · 100% ok · avg 500ms · last nr_tts m/a 500ms",
    );
  });
  test("records without timing omit avg and latency", () => {
    ...
    expect(formatUsageSummary(path)).toBe(
      "Usage: 1 call · 100% ok · last nr_web_search",
    );
  });
});

describe("formatUsageByTool per-tool breakdown", () => {
  test("undefined on an empty log", () => {
    const dir = mkdtempSync(join(tmpdir(), "nr-test-"));
    const path = join(dir, "9router-usage.jsonl");
    expect(formatUsageByTool(path)).toBeUndefined();
  });
  test("one line per tool: count, ok-rate, avg latency; most recent tool first", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nr-test-"));
    const path = join(dir, "9router-usage.jsonl");
    // Image: 2 calls 1 ok, avg 1.5s · TTS: 1 call 0 ok, no ms → no avg.
    // 2ms gaps make the ts ordering deterministic (same-ms ties sort alphabetically).
    const gap = () => new Promise((r) => setTimeout(r, 2));
    logUsage({ tool: "nr_image_generate", ms: 1000, ok: true }, path);
    await gap();
    logUsage({ tool: "nr_tts", ok: false }, path);
    await gap();
    logUsage({ tool: "nr_image_generate", ms: 2000, ok: false }, path);
    const lines = formatUsageByTool(path)!;
    expect(lines.length).toBe(2);
    // Image logged last → first row (most recently used tool first)
    expect(lines[0]).toContain("nr_image_generate");
    expect(lines[0]).toContain("2 calls");
    expect(lines[0]).toContain("50% ok");
    expect(lines[0]).toContain("avg 1.5s");
    expect(lines[1]).toContain("nr_tts");
    expect(lines[1]).toContain("1 calls");
    expect(lines[1]).toContain("0% ok");
    expect(lines[1]).not.toContain("avg");
  });
  test("corrupt lines are skipped like readUsageRecords", () => {
    const dir = mkdtempSync(join(tmpdir(), "nr-test-"));
    const path = join(dir, "9router-usage.jsonl");
    logUsage({ tool: "nr_embed", ms: 10, ok: true }, path);
    writeFileSync(path, readFileSync(path, "utf-8") + "garbage\n");
    const lines = formatUsageByTool(path)!;
    expect(lines.length).toBe(1);
    expect(lines[0]).toContain("nr_embed");
  });
});
...
describe("readUsageRecords tail + shape guard", () => {
  test("maxLines reads the tail only", () => {
    ...
    expect(recs.map((r) => (r as unknown as { seq: number }).seq)).toEqual([
      3, 4,
    ]);
  });
